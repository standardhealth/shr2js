...
const {SHRParser} = require('./parsers/SHRParser');
const {Namespace, DataElement, Concept, Cardinality, Identifier, IdentifiableValue, RefValue, Valueset, PrimitiveIdentifier, ChoiceValue, IncompleteValue, ValueSetConstraint, CodeConstraint, TypeConstraint, CardConstraint, TBD, PRIMITIVES} = require('../models');
...
class Importer extends SHRParserListener {
  ...
  enterConcepts(ctx) {
    ...
    enterValuesetDef(ctx) {
        ...
        const id = new Identifier(this._currentNs, name);
        const valueset = new Valueset(id);

        var path = this._preprocessedData.resolvePath('default', this._currentNs);
    ...
    enterValuesetValue(ctx) {
        ...
        if (ctx.valuesetInlineValue() != null) {
            result = this.processInlineValue(ctx.valuesetInlineValue());
        } else if (ctx.fullyQualifiedCode() != null){   // e.g. MTH#1234567
            result = this.processFullyQualifiedCode(ctx.fullyQualifiedCode());
        }
        this._currentDef.addCode(result);
    }

    enterValuesetProp(ctx) {
        var result = this.processFullyQualifiedCode(ctx.conceptProp().concepts().fullyQualifiedCode()[0]);
        this._currentDef.addConcept(result);
    }

    enterValuesetDescendingFrom(ctx) {
        //KW_INCLUDES_CODES_DESCENDING_FROM fullyQualifiedCode (KW_AND_NOT_DESCENDING_FROM fullyQualifiedCode)*;
        var result = this.processFullyQualifiedCode(ctx.fullyQualifiedCode()[0]);

        if (typeof result != 'undefined') {
            if (ctx.KW_INCLUDES_CODES_DESCENDING_FROM() != '') {
                result.descendingFrom = true;
                this._currentDef.addRule(result);
            }
            if (ctx.KW_AND_NOT_DESCENDING_FROM() != '') {
                result = this.processFullyQualifiedCode(ctx.fullyQualifiedCode()[1]);
    ...
    enterValuesetFrom(ctx) {
      //KW_INCLUDES_CODES_FROM fullyQualifiedCode;
        var result = this.processFullyQualifiedCode(ctx.fullyQualifiedCode());
        if (typeof result != 'undefined') {
            if (ctx.KW_INCLUDES_CODES_FROM() != '') {
  ...
  processFullyQualifiedCode(ctx) {
    ...
    processInlineValue(ctx) {
        var cs;
        const csAlias = ctx.CODE().getText();
        const resolution = this._preprocessedData.resolvePath('default', this._currentNs);
        if (resolution.error) {
            this.addError(resolution.error);
            cs = resolution.url ? resolution.url : csAlias;
        } else {
