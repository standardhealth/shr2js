...
const {SHRParser} = require('./parsers/SHRParser');
const {Namespace, DataElement, Concept, Cardinality, Identifier, IdentifiableValue, RefValue, Valueset, ValuesetConceptRule, PrimitiveIdentifier, ChoiceValue, IncompleteValue, ValueSetConstraint, CodeConstraint, TypeConstraint, CardConstraint, TBD, PRIMITIVES} = require('../models');

class Importer extends SHRParserListener {
  constructor(preprocessedData, preprocessedVSData) {
    super();
    ...
    // The preprocessed data indicating valid elements and vocabularies, for use with valuesets only
    this._preprocessedVSData = preprocessedVSData;
    // The map of namespace to elements
    this._nsMap = {};
    // The currently active namespace
    this._currentNs = '';
    // The currently active valueset namespace
    this._currentVSNs = '';
  ...
  exitElementDef(ctx) {
    this.pushCurrentDefinition(false);
  }
  ...
  enterConcepts(ctx) {
    ...
    for (const fqc of ctx.fullyQualifiedCode()) {
      this._currentDef.addConcept(this.processFullyQualifiedCode(fqc, false));
    }
  }

  // Valuesets
    enterValuesetDefsDoc(ctx) {
        const ns = ctx.valuesetDefsHeader().namespace().getText();
        this._currentVSNs = ns;
        if (typeof this._nsMap[ns] == 'undefined') {
            this._nsMap[ns] = new Namespace(ns);
        }
    }
    ...
    enterValuesetDef(ctx) {
        const name = ctx.valuesetHeader().simpleName().getText();
        const id = new Identifier(this._currentVSNs, name);
        const valueset = new Valueset(id);

        var path = this._preprocessedVSData.resolvePath('default', this._currentVSNs);

        valueset.url = path.url + name;
        valueset.name = name;

        this._currentDef = valueset;
    }

    exitValuesetDef(ctx) {
      this.pushCurrentDefinition(true);
    }

    enterValuesetValue(ctx) {
        const code = ctx.getText();
        var value;
        var result;
        if (ctx.valuesetInlineValue() != null) {
            result = this.processInlineValue(ctx.valuesetInlineValue(), true);
        } else if (ctx.fullyQualifiedCode() != null){   // e.g. MTH#1234567
            result = this.processFullyQualifiedCode(ctx.fullyQualifiedCode(), true);
        }
        this._currentDef.addCode(result);
    }

    enterValuesetProp(ctx) {
        for (const fqc of ctx.conceptProp().concepts().fullyQualifiedCode()) {
            this._currentDef.addConcept(this.processFullyQualifiedCode(fqc, true));
        }
    }
    ...
    enterValuesetDescendingFrom(ctx) {
      var ctr = 0;
        for (const fqc of ctx.fullyQualifiedCode()) {
          var result = this.processFullyQualifiedCode(fqc, true);
          if (ctr == 0) {
              result.descendingFrom = true;
          } else {
              result.descendingFrom = false;
          }
          this._currentDef.addRule(result);
          ctr++;
        }
    }

    enterValuesetFrom(ctx) {
      var result = this.processFullyQualifiedCode(ctx.fullyQualifiedCode(), true);
      result.includesFrom = true;
      this._currentDef.addRule(result);
    }
  ...
  processType(ctx, min, max) {
    if (ctx.elementWithConstraint()) {
      ...
      if (cst.elementTypeConstraint()) {
        ...
        const code = this.processFullyQualifiedCode(cst.elementCodeValueConstraint().fullyQualifiedCode(), false);
        value.addConstraint(new CodeConstraint(code, path));
      } else if (cst.elementWithUnitsConstraint()) {
        const code = this.processFullyQualifiedCode(cst.elementWithUnitsConstraint().fullyQualifiedCode(), false);
  ...
  processFullyQualifiedCode(ctx, isValuesetDoc) {
    var cs;
    const csAlias = ctx.ALL_CAPS().getText();
    const resolution = this.getPreprocessedData(isValuesetDoc).resolveVocabulary(csAlias, this.getNamespace(isValuesetDoc), ...this._usesNs);
    if (resolution.error) {
      this.addError(resolution.error);
    ...
    var concept;
    if (isValuesetDoc) {
      concept = new ValuesetConceptRule(cs, code);
    } else {
      concept = new Concept(cs, code);
    }
    ...
    processInlineValue(ctx, isValuesetDoc) {
      var cs;

      const csAlias = ctx.CODE().getText();
      const resolution = this.getPreprocessedData(isValuesetDoc).resolvePath('default', this.getNamespace(isValuesetDoc));
      if (resolution.error) {
          this.addError(resolution.error);
          cs = resolution.url ? resolution.url : csAlias;
      } else {
      ...
      var concept;
      if (typeof this._currentDef == 'Valueset') {
          concept = new ValuesetConceptRule(cs, code);
      } else {
          concept = new Concept(cs, code);
      }
  ...
  pushCurrentDefinition(isValuesetDoc) {
    this._nsMap[this.getNamespace(isValuesetDoc)].addDefinition(this._currentDef);
    this._currentDef = null;
  }
  ...
  getPreprocessedData(isValuesetDoc) {
    if (isValuesetDoc) {
        return this._preprocessedVSData;
    } else {
        return this._preprocessedData;
    }
  }

  getNamespace(isValuesetDoc) {
    if (isValuesetDoc) {
        return this._currentVSNs;
    } else {
