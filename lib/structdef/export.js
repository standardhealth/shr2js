const {DataElement, Group, Value, CodeFromValueSetValue, RefValue, OrValues, QuantifiedValue} = require('../models');
...
function exportToStructureDefinitions(namespaces) {
  ...
  for (const ns of namespaces) {
    for (const def of ns.definitions) {
      if (def.isEntry) {
        try {
          structdefs.push(new StructureDefinitionExporter(def.identifier, nsMap).export());
        } catch(e) {
          console.error(`Failed to export ${def.identifier.fqn} to a structure definition\n  `, e);
        }
      }
    }
  }
...
class StructureDefinitionExporter {
  ...
  addPrimitiveValue(value, path, min=1, max='1') {
    ...
    if (value instanceof CodeFromValueSetValue && typeof value.valueset != 'undefined' && value.valueset.length > 0) {
      obj.binding = {
        strength: 'required',
        valueSetReference: {
