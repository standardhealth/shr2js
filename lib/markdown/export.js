...
class MarkdownExporter {
    ...
    if (def instanceof Group) {
      for (const el of def.elements) {
        if (el.value instanceof OrValues) {
          ...
          md += this.tr(this.identifierMD(el), this.cardinalityMD(el), this.elementDescription(el));
        }
      }
    } else {
      md += this.tr('`Value`', this.valueMD(def.value));
    }
  ...
  choiceTr(el, indent=0) {
    const indentIt = function(text, indent) {
    ...
    let md = this.tr(indentIt('Choice', indent), this.cardinalityMD(el));
    for (let option of el.value.values) {
      ...
      if (option.value instanceof OrValues) {
        ...
        md += this.tr(indentIt(this.identifierMD(option), indent+1), this.cardinalityMD(option), this.elementDescription(option));
      }
    }
    return md;
  }
