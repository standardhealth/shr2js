    ...
    for (const el of def.elements) {
      if (el.value instanceof OrValues) {
        ...
        for (const option of el.value.values) {
          const optID = option instanceof QuantifiedValue ? option.value.identifier : option.identifier;
          md += `| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\\|&nbsp;${identifierToMarkdown(optID)} | ${quantifiedValueToMarkdown(option)} | ${elementDescription(option, dictionary)} |\n`;
        }
      } else {
...
function nontabularChoice(value) {
  let md = `Choice of: <ul>`;
  for (const option of value.values) {
    const quantifier = quantifiedValueToMarkdown(option);
    md += `<li>${quantifier == 1? '' : quantifier + ' '}${valueToMarkdown(option)}</li>`;
  }
  return md + '</ul>';
}
...
function toURLRelativeToNamespace(identifier) {
  // TODO: This makes a big assumption that all namespaces are siblings to each other!
