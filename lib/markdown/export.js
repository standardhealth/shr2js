...
const {IdentifiableValue, RefValue, ChoiceValue, TBD} = require('../models');
...
class MarkdownExporter {
  ...
  valueMD(value) {
    ...
    const vsConstraint = value.constraintsFilter.own.valueSet.single;
    if (vsConstraint) {
      md += ` from ${vsConstraint.valueSet}`;
    }
    return md;
  }
...
function nsTitle(namespace) {
  ...
  if (lastDot >= 0) {
    namespace = namespace.substr(lastDot + 1);
  }
