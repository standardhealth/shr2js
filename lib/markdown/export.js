...
function exportToHTML(namespaces) {
  ...
  for (const ns of Object.keys(md.namespaces)) {
    const namespaceHTML = embedInHTMLPage(`SHR: ${ns}`, md2html.render(md.namespaces[ns].index));
    const definitionHTMLs = {};
    for (const def of Object.keys(md.namespaces[ns].definitions)) {
      definitionHTMLs[def] = embedInHTMLPage(def, md2html.render(md.namespaces[ns].definitions[def]));
    }
    namespaceHTMLs[ns] = {
      index: namespaceHTML,
...
function embedInHTMLPage(title, body) {
