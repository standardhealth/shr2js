    ...
    for (const el of def.elements) {
      if (el.value instanceof OrValues) {
        ...
        for (const option of el.value.values) {
          md += `|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\\| [\`${option.identifier.name}\`](#bar) | ${quantifiedValueToMarkdown(option)} | ${elementDescription(option, dictionary)} |\n`;
        }
    } else {



        md += `| [\`${el.value.identifier.name}\`](../${el.value.identifier.namespace.split('.').splice(-1)}/#${el.value.identifier.name}) | ${quantifiedValueToMarkdown(el)} | ${elementDescription(el, dictionary)} |\n`;
      }
    }
  } else {
...
function typeToMarkdown(value) {
  ...
  return `[${value.identifier.name}](../${value.identifier.namespace.split('.').splice(-1)}/#${value.identifier.name})`;
}
