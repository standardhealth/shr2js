...
//   <label> else <codesystem>":"<code>
// are possible. This one means use label attribute value if that exists else use the codesystem attribute value followed by a colon then the 
// code attribute value. Another example:
...
// *Concept				= { name: <label> else <codesystem>":"<code>, type: "Concept", codesystem: <codesystem>, code: <code>, url: <url for concept> }
  const result = {};
  if (concept.label) {
	  result["name"] = concept.label;
  } else {
	  result["name"] = `${concept.codesystem}:${concept.code}`
  }
  result["type"] = "Concept";
